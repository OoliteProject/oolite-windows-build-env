...
#include <string.h>
#include <stdlib.h>
#include <pthread20040622.h>
#include <semaphore20040622.h>
#include <sys/time.h>
#include <errno.h>
...
//#undef SHOW
//#define SHOW(x,...)	do {fprintf(stdout, x, __VA_ARGS__);} while (0)
...
ENTER("sleep_until_timeout_or_stop_request");
	...
	SHOW("polling_thread > sleep_until_timeout_or_stop_request > stop sem_timedwait %ld.%09lu -- err=%d, errno=%u\n", 
       tv.tv_sec, tv.tv_usec*1000, err, errno);
...
#ifdef PTW32_VERSION	// pthreadsGC2
#warning using pthreadsGC2
	if (my_thread.p)
#else
	if (my_thread)
#endif
